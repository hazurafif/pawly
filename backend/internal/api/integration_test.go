...
func TestTwoDevicesConverge(t *testing.T) {
	...
	// Phone A: offline edits — a pet and a milestone event.
	phoneAEdits := map[string][]map[string]any{
		...
		"events": {{
			"id": "e-1", "pet_id": "pet-1", "kind": "milestone", "title": "First mouse",
	...
	// Phone B edits: renames the pet and adds a feed event.
	phoneBEdits := map[string][]map[string]any{
		...
		"events": {{
			"id": "e-2", "pet_id": "pet-1", "kind": "feed", "title": "",
			"occurred_at": "2030-07-28T00:00:00.000Z",
	...
	want := map[string]int{"pets": 1, "events": 2}
	for _, ph := range []*store.Store{phoneA, phoneB} {
		...
		pet := got["pets"][0]
		if pet["name"] != "Miko (Bella)" {
			t.Fatalf("pet name %q, want Miko (Bella)", pet["name"])
		}
		if len(got["reminder_rules"]) != 0 {
			t.Fatalf("unexpected reminder rules: %v", got["reminder_rules"])
		}
	}
}
