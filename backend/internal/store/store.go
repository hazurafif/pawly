...
var TableNames = []string{"pets", "events", "photos", "reminder_rules"}
...
var pushOrder = []string{"pets", "events", "photos", "reminder_rules"}
...
var migrations = []string{
	// 0: initial schema (v2 fresh build)
	`
CREATE TABLE pets (
	id TEXT PRIMARY KEY,
	name TEXT NOT NULL,
...
// TombstonedPhotoIDs returns ids of photo rows that are soft-deleted. Their
// binaries are no longer fetchable and can be removed from disk.
func (s *Store) TombstonedPhotoIDs() ([]string, error) {
	rows, err := s.db.Query(`SELECT id FROM photos WHERE deleted_at IS NOT NULL`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
